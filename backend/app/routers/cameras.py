...
def get_py_video_stream(id: int, cam_ref: int, av_video_stream):
  py_video_stream = VideoStream(id=id, camera_ref=cam_ref, codec=av_video_stream.codec.name,
                                time_base=(av_video_stream.time_base.numerator,
                                            av_video_stream.time_base.denominator),
                                height=av_video_stream.height, width=av_video_stream.width,
                                sample_aspect_ratio=(av_video_stream.sample_aspect_ratio.numerator,
...
async def add_camera(py_cam_in: CameraIn):
  ...
  try:
    ...
    av_camera.close()
     ...
     return Response(content=f"Unable to connect to camera due to an exception: {e}.",
  ...
  if not created:
     return Response(content=f"Unable to start recording for camera at: {py_cam_in.url}. Exception: {err}",
                     status_code=500, media_type="text/plain")
